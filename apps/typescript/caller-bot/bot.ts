// ── DEXL Callers Bot — Telegram Bot (grammy) ──────────────────────────
...
export const poweredByHTMLText = '<a href="https://dexlens.io">Powered by Dexlens API</a>';
...
export function createBot(config: BotConfig): Bot {
  ...
  bot.command('start', async (ctx) => {
    await ctx.reply(
      dedent`👥 <b>DEXL Callers Bot</b>\n\nTrack calls, rank callers, and compete on leaderboards.

      Type /help for commands.
      Type /features for info about this bots features.
  ...
  bot.command('features', async (ctx) => {
    await ctx.reply(
      dedent`👥 <b>DEXL Callers Bot Features</b>\n\nTrack calls, rank callers, and compete on leaderboards.
      
<b>Call Tracking</b> — Paste a token CA, LP link, or chart URL → bot registers the call and tracks performance

<b>Auto Calls / Button Mode</b> — Auto-register after 30s, or manual confirm with alpha/gamble selection

<b>Leaderboards</b> — Top calls by multiplier, performance rankings by points (24h / 7d / 30d / all)

<b>Points System</b> — -1 (&lt; 1.5x) → 0 (1.5-2x) → +2 (2-5x) → +3 (5-15x) → +4 (15-30x) → +5 (30x+)

<b>Win Rate</b> — Percentage of calls hitting ≥ 2x

<b>Ranks</b> — Amateur → Novice → Contender → Guru → Oracle (based on win rate)

<b>PNL Cards</b> — Shareable images showing entry, ATH, gain

<b>Hardcore Mode</b> — Auto-kick members below minimum win rate

<b>Block/Unblock</b> — Admins can block users from making calls

<b>Multi-chain</b> — Solana, Ethereum, Base, BSC
...
${poweredByHTMLText}`,
  ...
  bot.command('help', async (ctx) => {
    await ctx.reply(
      [
        '👥 <b>DEXL Callers Commands</b>',
        '',
        '<b>Group:</b>',
        '/features - Information about the bot',
...
async function registerCall(
  ...
  try {
    ...
    let text = group?.display_mode === 'advanced'
    ...
    if (group?.call_channel_id) {
      try {
        await ctx.api.sendMessage(group.call_channel_id, text, { parse_mode: 'HTML', disable_web_page_preview: true });
      } catch (err) {
